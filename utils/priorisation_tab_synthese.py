"""Onglet 3 — Synthèse / tableau de bord."""
...
def render_action_card(carte: dict) -> None:
    ...
    with st.container(border=True, height="stretch"):
        ...
        if description:
            st.caption(short_description(description, CARD_DESCRIPTION_MAX_LEN))
...
""",
    ...
    selected_id = ctx.collectivite_id
    leviers = ctx.leviers_notes
    notes = ctx.notes_perimetre
    ids_by_case = ctx.ids_by_case
    reductions = ctx.reductions
    weights = ctx.weights
    hors_competence = ctx.exclusions
    nom_par_id = ctx.nom_par_id
    df_fiches_action = ctx.df_fiches_action
    df_priorisation_all = ctx.df_priorisation_all
    ...
    cartes_actions = load_cartes_actions(
        selected_id,
        reductions,
        weights,
        nom_par_id,
    ...
    if not cartes_actions:
        st.info(
            "Aucune action sauvegardée. "
            "Rendez-vous à l'onglet « Actions de référence » pour constituer "
            "votre sélection.",
    ...
    if not volets_a_discuter:
        st.info(
            ...
            "l'onglet « Priorisation des actions » pour signaler les volets qui "
        ...
        if treemap_children:
            ...
            with _detail_slot_holder["slot"].container():
                if selection:
                    ...
                    if not action_ids:
                        st.info(
                            f"Aucune action associée à : {levier} · {cat_label}."
                        )
                    else:
                        ...
                        if df_fiches_case.empty:
                            st.info(
                                "Aucune fiche action trouvée pour les "
                                "identifiants associés."
                            )
                        else:
                            for _, fiche in df_fiches_case.iterrows():
                                titre = (
                                    fiche.get("titre") or f"Fiche #{fiche['id']}"
                                )
                    ...
                    with st.expander("Voir les actions des autres collectivités"):
                        if cat_id is None:
                            ...
                            df_priorisation_autres = df_priorisation_all[
                                ...
                                & (
                                    df_priorisation_all["collectivite_id"]
                                    != selected_id
                                )
                            ]
                            collectivites_avec_actions: list[
                                tuple[int, list[int]]
                            ] = []
                            ...
                            if not collectivites_avec_actions:
                                ...
                                collectivites_avec_actions.sort(
                                    key=lambda item: nom_par_id.get(
                                        item[0], ""
                                    ).lower()
                                )
                                affiche = False
                                for ct_id, ct_action_ids in collectivites_avec_actions:
                                    df_fiches_ct = df_fiches_action[
                                        ...
                                        & (
                                            df_fiches_action["collectivite_id"]
                                            == ct_id
                                        )
                                    ]
                                ...
                                if not affiche:
                                    st.info(
                                        "Des identifiants sont enregistrés pour "
                                        "d'autres collectivités, mais aucune fiche "
                                        "action correspondante n'a été trouvée."
                                    )
